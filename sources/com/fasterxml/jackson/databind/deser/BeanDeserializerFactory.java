    ...
    protected SettableAnyProperty constructAnySetter(DeserializationContext deserializationContext, BeanDescription beanDescription, AnnotatedMember annotatedMember) throws JsonMappingException {
        ...
        if (annotatedMember instanceof AnnotatedMethod) {
            ...
            if (type.isMapLikeType()) {
                ...
                BeanProperty.Std std2 = new BeanProperty.Std(PropertyName.construct(annotatedMember.getName()), resolveMemberAndTypeAnnotations, null, annotatedMember, PropertyMetadata.STD_OPTIONAL);
                javaType = contentType;
                std = std2;
            } else if (type.hasRawClass(JsonNode.class) || type.hasRawClass(ObjectNode.class)) {
        ...
        if (z) {
            return SettableAnyProperty.constructForMapField(deserializationContext, std, annotatedMember, javaType, keyDeserializer, jsonDeserializer, typeDeserializer);
        }
        return SettableAnyProperty.constructForMethod(deserializationContext, std, annotatedMember, javaType, keyDeserializer, jsonDeserializer, typeDeserializer);
    }
