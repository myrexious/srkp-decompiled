...
public class Processor implements ExecutionListener, ForegroundProcessor {
    ...
    public boolean startWork(String id2) {
        return startWork(id2, null);
    }

    public boolean startWork(String id2, WorkerParameters.RuntimeExtras runtimeExtras) {
        synchronized (this.mLock) {
            if (isEnqueued(id2)) {
                Logger.get().debug(TAG, String.format("Work %s is already enqueued for processing", id2), new Throwable[0]);
                return false;
            }
            WorkerWrapper build = new WorkerWrapper.Builder(this.mAppContext, this.mConfiguration, this.mWorkTaskExecutor, this, this.mWorkDatabase, id2).withSchedulers(this.mSchedulers).withRuntimeExtras(runtimeExtras).build();
            ListenableFuture<Boolean> future = build.getFuture();
            future.addListener(new FutureListener(this, id2, future), this.mWorkTaskExecutor.getMainThreadExecutor());
            this.mEnqueuedWorkMap.put(id2, build);
            this.mWorkTaskExecutor.getBackgroundExecutor().execute(build);
            Logger.get().debug(TAG, String.format("%s: processing %s", getClass().getSimpleName(), id2), new Throwable[0]);
            return true;
        }
    }
    ...
    public boolean stopForegroundWork(String id2) {
        boolean interrupt;
        synchronized (this.mLock) {
            Logger.get().debug(TAG, String.format("Processor stopping foreground work %s", id2), new Throwable[0]);
            interrupt = interrupt(id2, this.mForegroundWorkMap.remove(id2));
        }
        return interrupt;
    }

    public boolean stopWork(String id2) {
        boolean interrupt;
        synchronized (this.mLock) {
            Logger.get().debug(TAG, String.format("Processor stopping background work %s", id2), new Throwable[0]);
            interrupt = interrupt(id2, this.mEnqueuedWorkMap.remove(id2));
        }
        return interrupt;
    }

    public boolean stopAndCancelWork(String id2) {
        boolean interrupt;
        synchronized (this.mLock) {
            boolean z = true;
            Logger.get().debug(TAG, String.format("Processor cancelling %s", id2), new Throwable[0]);
            this.mCancelledIds.add(id2);
            WorkerWrapper remove = this.mForegroundWorkMap.remove(id2);
            if (remove == null) {
                z = false;
            }
            if (remove == null) {
                remove = this.mEnqueuedWorkMap.remove(id2);
            }
            interrupt = interrupt(id2, remove);
            if (z) {
    ...
    public boolean isCancelled(String id2) {
        boolean contains;
        synchronized (this.mLock) {
            contains = this.mCancelledIds.contains(id2);
        }
        return contains;
    }
    ...
    private static boolean interrupt(String id2, WorkerWrapper wrapper) {
        if (wrapper != null) {
            wrapper.interrupt();
            Logger.get().debug(TAG, String.format("WorkerWrapper interrupted for %s", id2), new Throwable[0]);
            return true;
        }
        Logger.get().debug(TAG, String.format("WorkerWrapper could not be found for %s", id2), new Throwable[0]);
        return false;
    }
