...
public class TypeUtils {
    ...
    private static void appendRecursiveTypes(StringBuilder sb, int[] iArr, Type[] typeArr) {
        for (int i = 0; i < iArr.length; i++) {
            appendAllTo(sb.append(Typography.less), ", ", typeArr[i].toString()).append(Typography.greater);
        }
        Type[] typeArr2 = (Type[]) ArrayUtils.removeAll((Object[]) typeArr, iArr);
        if (typeArr2.length > 0) {
            appendAllTo(sb.append(Typography.less), ", ", typeArr2).append(Typography.greater);
        }
    }
    ...
    private static boolean isAssignable(Type type, ParameterizedType parameterizedType, Map<TypeVariable<?>, Type> map) {
        ...
        for (TypeVariable<?> typeVariable : typeArguments2.keySet()) {
            ...
            if (unrollVariableAssignments != null || !(unrollVariableAssignments2 instanceof Class)) {
                if (unrollVariableAssignments2 != null && unrollVariableAssignments != null && !unrollVariableAssignments.equals(unrollVariableAssignments2) && (!(unrollVariableAssignments instanceof WildcardType) || !isAssignable(unrollVariableAssignments2, unrollVariableAssignments, map))) {
                    return false;
                }
            }
        }
    ...
    private static String parameterizedTypeToString(ParameterizedType parameterizedType) {
        ...
        if (findRecursiveTypes.length > 0) {
            ...
            appendAllTo(sb.append(Typography.less), ", ", parameterizedType.getActualTypeArguments()).append(Typography.greater);
        }
        return sb.toString();
    }
