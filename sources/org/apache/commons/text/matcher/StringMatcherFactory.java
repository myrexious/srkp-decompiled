...
public final class StringMatcherFactory {
    ...
    public StringMatcher stringMatcher(char... cArr) {
        StringMatcher charArrayMatcher;
        int length = ArrayUtils.getLength(cArr);
        if (length == 0) {
            return NONE_MATCHER;
        }
        if (length == 1) {
            charArrayMatcher = new AbstractStringMatcher.CharMatcher(cArr[0]);
        } else {
            charArrayMatcher = new AbstractStringMatcher.CharArrayMatcher(cArr);
        }
