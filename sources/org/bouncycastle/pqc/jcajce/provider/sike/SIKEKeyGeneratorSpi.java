...
public class SIKEKeyGeneratorSpi extends KeyGeneratorSpi {
    ...
    protected SecretKey engineGenerateKey() {
        ...
        if (kEMGenerateSpec != null) {
            SecretWithEncapsulation generateEncapsulated = new SIKEKEMGenerator(this.random).generateEncapsulated(((BCSIKEPublicKey) kEMGenerateSpec.getPublicKey()).getKeyParams());
        ...
        SIKEKEMExtractor sIKEKEMExtractor = new SIKEKEMExtractor(((BCSIKEPrivateKey) this.extSpec.getPrivateKey()).getKeyParams());
        byte[] encapsulation = this.extSpec.getEncapsulation();
        byte[] extractSecret = sIKEKEMExtractor.extractSecret(encapsulation);
