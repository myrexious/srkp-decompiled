...
class NTRUSampling {
    ...
    public HPSPolynomial sampleFixedType(byte[] bArr) {
        ...
        while (true) {
            ...
            int i5 = i4 * 4;
            int i6 = i4 * 15;
            iArr[i5 + 0] = ((bArr[i6 + 0] & UByte.MAX_VALUE) << 2) + ((bArr[i6 + 1] & UByte.MAX_VALUE) << 10) + ((bArr[i6 + 2] & UByte.MAX_VALUE) << 18) + ((bArr[i6 + 3] & UByte.MAX_VALUE) << 26);
            iArr[i5 + 1] = (((bArr[(i4 * 3) + 15] & UByte.MAX_VALUE) & 192) >> 4) + ((bArr[i6 + 4] & UByte.MAX_VALUE) << 4) + ((bArr[i6 + 5] & UByte.MAX_VALUE) << 12) + ((bArr[i6 + 6] & UByte.MAX_VALUE) << 20) + ((bArr[i6 + 7] & UByte.MAX_VALUE) << 28);
            int i7 = (((bArr[(i4 * 7) + 15] & UByte.MAX_VALUE) & 240) >> 2) + ((bArr[i6 + 8] & UByte.MAX_VALUE) << 6) + ((bArr[i6 + 9] & UByte.MAX_VALUE) << 14) + ((bArr[i6 + 10] & UByte.MAX_VALUE) << 22);
            byte b = bArr[i6 + 11];
            iArr[i5 + 2] = i7 + ((b & UByte.MAX_VALUE) << 30);
            iArr[i5 + 3] = (b & UByte.MAX_VALUE & 252) + ((bArr[i6 + 12] & UByte.MAX_VALUE) << 8) + ((bArr[i6 + 13] & UByte.MAX_VALUE) << 16) + ((bArr[i6 + 14] & UByte.MAX_VALUE) << 24);
            i4++;
        }
        int i8 = i * 4;
        if (i3 > i8) {
            int i9 = i * 15;
            iArr[i8 + 0] = ((bArr[i9 + 0] & UByte.MAX_VALUE) << 2) + ((bArr[i9 + 1] & UByte.MAX_VALUE) << 10) + ((bArr[i9 + 2] & UByte.MAX_VALUE) << 18) + ((bArr[i9 + 3] & UByte.MAX_VALUE) << 26);
            iArr[i8 + 1] = (((bArr[(i * 3) + 15] & UByte.MAX_VALUE) & 192) >> 4) + ((bArr[i9 + 4] & UByte.MAX_VALUE) << 4) + ((bArr[i9 + 5] & UByte.MAX_VALUE) << 12) + ((bArr[i9 + 6] & UByte.MAX_VALUE) << 20) + ((bArr[i9 + 7] & UByte.MAX_VALUE) << 28);
        }
        int i10 = 0;
        while (true) {
            i2 = weight / 2;
    ...
    public HRSSPolynomial sampleIidPlus(byte[] bArr) {
        ...
        short s2 = (short) ((-((s & UShort.MAX_VALUE) >>> 15)) | 1);
        for (int i5 = 0; i5 < i; i5 += 2) {
            hRSSPolynomial.coeffs[i5] = (short) (hRSSPolynomial.coeffs[i5] * s2);
        }
        for (int i6 = 0; i6 < i; i6++) {
            hRSSPolynomial.coeffs[i6] = (short) (((hRSSPolynomial.coeffs[i6] & UShort.MAX_VALUE) ^ ((hRSSPolynomial.coeffs[i6] & UShort.MAX_VALUE) >>> 15)) & 3);
        }
