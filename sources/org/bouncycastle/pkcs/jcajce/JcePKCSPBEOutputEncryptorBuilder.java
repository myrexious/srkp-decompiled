        ...
        try {
            if (isPKCS12(this.algorithm)) {
                ...
                if (MiscObjectIdentifiers.id_scrypt.equals((ASN1Primitive) pBKDFConfig.getAlgorithm())) {
                    ...
                    algorithmIdentifier = new AlgorithmIdentifier(this.algorithm, createCipher.getParameters() != null ? new PBES2Parameters(new KeyDerivationFunc(MiscObjectIdentifiers.id_scrypt, scryptParams), new EncryptionScheme(this.keyEncAlgorithm, ASN1Primitive.fromByteArray(createCipher.getParameters().getEncoded()))) : new PBES2Parameters(new KeyDerivationFunc(MiscObjectIdentifiers.id_scrypt, scryptParams), new EncryptionScheme(this.keyEncAlgorithm)));
                    cipher = createCipher;
                } else {
                    ...
                    createCipher2.init(1, simplifyPbeKey(generateSecret2), this.random);
